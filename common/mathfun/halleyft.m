...
derivs = [0 1 2] + deriv;
    ...
    if(avoidNaN)
        ...
        if(nargout > 1)
%             v_hat = gpuArray(v_hat);
%             guess = gpuArray(guess);
%             guess_count = gpuArray(guess_count);
%             refined = gpuArray(refined);
...
%                     [refined_gpu,refined_derivs_gpu] = halleyft(gpuArray(v_hat(:,s)), gpuArray(guess(1:i,s)), true, deriv, TOL, maxIter, false, uniq);
%                     refined(1:i,s) = gather(refined_gpu);
%                     refined_derivs(1:i,s,:) = gather(refined_derivs_gpu);
                end
            end
            if(resort)
                for i=1:3
...
%             refined_derivs = gather(refined_derivs);
...
%     v_hat_now = v_hat(:,columnNotDone,:);
    % Obtain the value of the function first
%     new_guess_vals = interpft1([0 2*pi],v_hat_now(:,:,1),new_guess,'horner_freq');
    % Check if the value is closer to zero
    new_guess_is_better_now = abs(new_guess_vals(:,:,1)) < abs(guess_vals(:,:,1));
%     any_new_guess_is_better_now = any(new_guess_is_better_now);
    % If the new guess is closer to zero, then calculate the derivatives
%     new_guess_vals(:,any_new_guess_is_better_now,2:3) = interpft1([0 2*pi],v_hat_now(:,any_new_guess_is_better_now,2:3),repmat(new_guess(:,any_new_guess_is_better_now),xqrep-[0 0 1]),'horner_freq');
%     for g = 1:guess_sz(1)
%         new_guess_vals(g,new_guess_is_better_now(g,:),2:3) = interpft1([0 2*pi],v_hat_now(g,new_guess_is_better_now(g,:),2:3),repmat(new_guess(g,new_guess_is_better_now(g,:)),xqrep-[0 0 1]),'horner_freq');
%     end
