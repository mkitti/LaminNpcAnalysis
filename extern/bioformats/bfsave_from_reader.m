function bfsave_from_reader(varargin)
...
% Input check
ip = inputParser;
ip.addRequired('I', @(x) isa(x,'loci.formats.IFormatReader'));
...
if isempty(ip.Results.metadata)
    metadata = ip.Results.I.getMetadataStore;
...
%     plane = ip.Results.I(:, :, i, j, k)';
    writer.saveBytes(index-1, ip.Results.I.openBytes(index-1));
end
writer.close();
